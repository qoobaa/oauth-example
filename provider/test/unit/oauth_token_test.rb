...
class RequestTokenTest < ActiveSupport::TestCase
  ...
  def test_should_not_exchange_without_provided_verifier
    @token.authorize!(users(:quentin))
    assert !@token.exchange!
  end

  def test_should_exchange_with_provided_verifier
    @token.authorize!(users(:quentin))
    ...
    assert !@access.invalidated?
  end

  def test_should_find_token_by_authorized_token_key
    @token.authorize!(users(:quentin))
    found_token =  OauthToken.find_token(@token.token)
    assert found_token
    assert_not_nil found_token.client_application
  end

  def test_should_not_find_token_by_invalid_token_key
    assert_nil OauthToken.find_token("")
  end

  def test_should_not_find_not_authorized_token
    assert_nil OauthToken.find_token(@token.token)
  end
